...
| **Éditable, effet différé (documenté dans l'UI)** | `rag_chunk_max_tokens`, `rag_chunk_overlap` (n'affectent que les *futures* ingestions) ; `llm_vendor`, `llm_model`, `base_url` (le client LLM en cache est invalidé à la sauvegarde, effet dès le *prochain* message, pas le tour en cours) | Formulaire complet + badge d'avertissement explicite |
...
7. **Addendum 2 — catalogue de fournisseurs avec modèles suggérés** : suite à la demande "un menu déroulant avec tous les modèles de chaque fournisseur", `llm_provider_kind` (famille de client générique) est devenu `llm_vendor` (fournisseur commercial concret : `ollama`/`openai`/`deepseek`/`kimi`/`anthropic`), chacun portant une URL par défaut et une liste de modèles suggérés (`app/agent/providers/catalog.py`). Le champ modèle reste en saisie libre (`<input list>` HTML natif) pour ne jamais bloquer sur un modèle plus récent absent de la liste. Un vrai bug de résolution d'URL a été trouvé et corrigé au passage : sans override, `base_url` retombait sur l'URL Ollama même pour un fournisseur distant — corrigé en ne défaultant plus sur `Settings.llm_base_url` que pour Ollama spécifiquement (voir `get_llm_client()`).
